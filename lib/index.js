...
export class Kadira extends EventEmitter2 {
  ...
  async _initWebSocket() {
    try {
      this._ws = await connectWithRetry(this._options.endpoint, {
        ...this._headers,
      ...
      this._ws.on('close', () => this.emit(CoreEvent.WEBSOCKET_CLOSED));

      this.emit(CoreEvent.WEBSOCKET_CONNECTED);
    } catch (e) {
      console.error('Monti APM WebSocket: Failed to connect', e);
    }
  }
  ...
  async _checkAuth() {
    ...
    Promise.resolve(this._initWebSocket());
