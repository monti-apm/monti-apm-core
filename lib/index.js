...
import { axiosRetry, hasCircularReference } from './utils';
...
export class Kadira {
  ...
  async sendData(_payload) {
    ...
    if (this._supportsJsonSeq) {
      return this._sendJsonStream(url, { data: payload });
    }
  ...
  _checkAuth() {
    ...
    return axiosRetry(uri, params).then(res => {
      const allowedContentTypes = res.headers['content-type'];

      if (typeof allowedContentTypes === 'string' &&
        allowedContentTypes.includes(ContentType.JSON_SEQ)) {
        this._supportsJsonSeq = true;
      }
  ...
  _sendJsonStream(url, params) {
    ...
    return axiosRetry(url, {
      ...params,
      data: stringifyStream(params.data),
      headers: {
        ...this._headers,
        ...params.headers,
