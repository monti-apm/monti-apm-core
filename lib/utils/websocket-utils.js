...
export async function connectWithBackoff(core) {
  ...
  const fibBackoff = backoff.exponential({
  ...
  const connect = async () => {
    try {
      ...
      ws.on(WebSocketEvent.CLOSE, (event) => {
        fibBackoff.backoff(event);
      });
    } catch (error) {
      fibBackoff.backoff(error);
    }
  };

  fibBackoff.failAfter(connectWithBackoff._failAfter);

  fibBackoff.on(BackoffEvent.READY, async (number, delay) => {
    core.emit(CoreEvent.WEBSOCKET_BACKOFF_READY, number, delay);
  });

  fibBackoff.on(BackoffEvent.BACKOFF, async (number, delay, error) => {
    // eslint-disable-next-line no-console,max-len
    console.log(`Monti APM WebSocket: Reconnection attempt #${number + 1} with a delay of ${delay}ms`);
    core.emit(CoreEvent.WEBSOCKET_BACKOFF, number, delay, error);
    await sleep(delay);
    await connect();
  });

  fibBackoff.on(BackoffEvent.FAIL, () => {
    // eslint-disable-next-line no-console,max-len
    console.error('Monti APM WebSocket: Reconnection Failed (Exhausted Backoff)');
